-- ============================================================================
-- FIX EMPTY RESULTS - Fixed Version (JSON Syntax Error Resolved)
...
-- Using jsonb_build_object to avoid JSON quote escaping issues
DO $$
DECLARE
  json_content jsonb;
BEGIN
  -- Build JSON content using jsonb functions to avoid quote issues
  json_content := jsonb_build_object(
    ...
    'content', jsonb_build_array(
      jsonb_build_object(
        ...
        'content', jsonb_build_array(
          jsonb_build_object('type', 'text', 'text', 'Getting Started with Faceless Content')
        )
      ),
      jsonb_build_object(
        'type', 'paragraph',
        'content', jsonb_build_array(
          jsonb_build_object('type', 'text', 'text', 'Faceless content creation is a growing trend that allows creators to build successful businesses without showing their face. This guide will walk you through the fundamentals.')
        )
      )
    )
  );

  -- Insert article only if no published articles exist
  IF NOT EXISTS (SELECT 1 FROM public.articles WHERE status = 'published' LIMIT 1) THEN
...
-- If guides exist but published = false, uncomment and run:
