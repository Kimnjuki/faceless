...
// Enhanced markdown components with inline images
const markdownComponents = {
  // Custom image component that uses themed content creation images
  img: ({ src, alt, ...props }: any) => {
    // If no src or broken src, use themed fallback
    if (!src || (typeof src === 'string' && src.includes('placeholder'))) {
      const randomCategory = ['writing', 'ai', 'monetization', 'productivity', 'privacy'][Math.floor(Math.random() * 5)];
      return (
        <div className="my-6">
          <ArticleImage
            src={null}
            alt={alt || 'Content creation image'}
  ...
  h1: ({ children, ...props }: any) => (
    <h1 className="text-3xl md:text-4xl font-bold mb-6 mt-8 text-foreground" {...props}>
      {children}
    </h1>
  ),
  h2: ({ children, ...props }: any) => (
    <h2 className="text-2xl md:text-3xl font-bold mb-4 mt-8 text-foreground" {...props}>
      {children}
    </h2>
  ),
  h3: ({ children, ...props }: any) => (
    <h3 className="text-xl md:text-2xl font-bold mb-3 mt-6 text-foreground" {...props}>
      {children}
    </h3>
  ),
  // Enhanced paragraphs
  p: ({ children, ...props }: any) => (
    <p className="mb-4 leading-relaxed text-foreground/90 text-lg" {...props}>
      {children}
    </p>
  ),
  // Enhanced lists
  ul: ({ children, ...props }: any) => (
    <ul className="mb-4 space-y-2 list-disc list-inside text-foreground/90" {...props}>
      {children}
    </ul>
  ),
  ol: ({ children, ...props }: any) => (
    <ol className="mb-4 space-y-2 list-decimal list-inside text-foreground/90" {...props}>
      {children}
    </ol>
  ),
  // Enhanced blockquotes
  blockquote: ({ children, ...props }: any) => (
    <blockquote className="border-l-4 border-primary/30 pl-4 my-6 italic text-foreground/80 bg-muted/30 py-2 rounded-r" {...props}>
      {children}
    </blockquote>
  ),
  ...
  code: ({ inline, children, ...props }: any) => {
    if (inline) {
      return (
        <code className="bg-muted px-2 py-1 rounded text-sm font-mono text-foreground/90" {...props}>
          {children}
        </code>
      );
    }
...
export function ArticleContentWithImages({ content }: { content: unknown }) {
  ...
  if (isTipTapFormat(parsedContent)) {
    ...
    if (htmlContent) {
      return (
        <div className={PROSE}>
          <ReactMarkdown components={markdownComponents}>{htmlContent}</ReactMarkdown>
        </div>
      );
    }
    return EMPTY_STATE;
    ...
    <>
      ...
      {hasIntro && (
        <div className={`${PROSE} mb-12 pb-8 border-b`}>
          {typeof intro === "string" && intro.length > 0 && (
            <>
              {isTipTapFormat(intro) ? (
                <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(typeof intro === "string" ? JSON.parse(intro) : intro)}</ReactMarkdown>
              ) : (
          ...
          {typeof intro === "object" && intro !== null && !Array.isArray(intro) && isTipTapFormat(intro) && (
            <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(intro)}</ReactMarkdown>
          )}
          {Array.isArray(intro) && intro.length > 0 && (
            <>
              {intro
                ...
                .map((p, i) => (
                  <div key={i} className={i === 0 ? "text-xl leading-relaxed text-foreground/90 font-light mb-4" : ""}>
                    {isTipTapFormat(p) ? (
                      <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(JSON.parse(p))}</ReactMarkdown>
                    ) : (
      ...
      {typeof body === "string" && body.length > 0 && (
        <div className={`${PROSE} mb-8`}>
          {isTipTapFormat(body) ? (
            <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(typeof body === "string" ? JSON.parse(body) : body)}</ReactMarkdown>
          ) : (
      ...
      {typeof body === "object" && body !== null && isTipTapFormat(body) && (
        <div className={`${PROSE} mb-8`}>
          <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(body)}</ReactMarkdown>
        </div>
      )}
      {tools.length > 0 && (
        ...
        sections.map((s, i) => {
          ...
          return (
            <section key={i} className="mb-12 scroll-mt-20">
              ...
              {sectionContent && (
                <div className={PROSE}>
                  <ReactMarkdown components={markdownComponents}>{sectionContent}</ReactMarkdown>
                </div>
              )}
            </section>
          );
      ...
      {typeof conclusion === "string" && conclusion.length > 0 && (
        <div className={`${PROSE} mt-8`}>
          {isTipTapFormat(conclusion) ? (
            <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(typeof conclusion === "string" ? JSON.parse(conclusion) : conclusion)}</ReactMarkdown>
          ) : (
            <ReactMarkdown components={markdownComponents}>{conclusion}</ReactMarkdown>
          )}
        </div>
      )}
      {typeof conclusion === "object" && conclusion !== null && isTipTapFormat(conclusion) && (
        <div className={`${PROSE} mt-8`}>
          <ReactMarkdown components={markdownComponents}>{convertTipTapToHTML(conclusion)}</ReactMarkdown>
        </div>
      )}
    </>
  );
